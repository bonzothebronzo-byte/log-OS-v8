  ...
  const setupConnectionListeners = (connection: any) => {
      connection.on('data', (data: P2PMessage) => {
          if (data.type === 'INIT_GAME' || data.type === 'TURN_COMMIT') {
             ...
             // Ensure turnCount is treated as a number
             const incomingTurn = typeof data.payload.turnCount === 'number' ? data.payload.turnCount : parseInt(data.payload.turnCount as any);
             setTurnCount(incomingTurn);
  ...
  const commitPlayerTurn = useCallback(() => {
    ...
    turnMoves.forEach(move => {
      ...
      newGrid[move.r][move.c] = { 
          ...newGrid[move.r][move.c], 
          isLocked: true,
          turnPlaced: turnCount // Ensure this is correctly set for all moved tiles, including resolved wildcards
      };
    });
